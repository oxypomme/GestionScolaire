...
var
  f_select_infraction: Tf_select_infraction;
...
procedure Tf_select_infraction.Init;
begin
...
procedure Tf_select_infraction.btn_rechercherClick(Sender: TObject);
begin
   btn_rechercher.visible := false;
   pnl_actif.enabled := false;
   if  pnl_tous_edit.Visible  then
       f_list_infraction.affi_data(modele.infraction_liste_tous)
   else if  pnl_num_edit.visible  then
        f_list_infraction.affi_data(modele.infraction_liste_num(edt_num.text))
   else if  pnl_periode_edit.visible  then
        f_list_infraction.affi_data(modele.infraction_liste_periode(DateToStr(edt_dtdeb.date)
				,DateToStr(edt_dtfin.date)))
   else if  pnl_immat_edit.visible  then
        f_list_infraction.affi_data(modele.infraction_liste_immat(edt_immat.text))
   else if  pnl_cond_edit.visible  then
        f_list_infraction.affi_data(modele.infraction_liste_cond(edt_permis.text,edt_nomcond.text))
   else if pnl_com_edit.visible  then
        f_list_infraction.affi_data(modele.infraction_liste_com(edt_insee.text,edt_nomcom.text));
end;

procedure   Tf_select_infraction.pnl_choix_btnClick (Sender : TObject);
var
   pnl : TPanel;
begin
   AucuneSelection;
   ...
   pnl	:= TPanel(f_select_infraction.FindComponent(pnl.name +'_edit'));
   style.panel_selection (pnl);
   pnl.show;
...
procedure   Tf_select_infraction.AucuneSelection;
begin
...
procedure  Tf_select_infraction.NonSelectionPanel (pnl : TPanel);
var
   pnl_enfant : TPanel;
begin
   style.panel_defaut(pnl);
   pnl_enfant	:= TPanel(f_select_infraction.FindComponent(pnl.name +'_btn'));
   style.panel_bouton(pnl_enfant);
   pnl_enfant	:= TPanel(f_select_infraction.FindComponent(pnl.name +'_edit'));
   pnl_enfant.Hide;
end;
